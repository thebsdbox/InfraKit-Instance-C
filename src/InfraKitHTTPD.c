...
{
    httpRequest *response = malloc(sizeof(httpRequest));
    response->messageBody = returnNewMessageBody(rawData);
    response->headers = returnNewHeaders(rawData, response);
    response->requestLine = returnNewRequestLine(rawData);
    parseRequestLine(response->requestLine, response);
    return response;
}
...
int receive(int socket)
{
    ...
    httpRequest *request = processHttpResponse(buffer);
...
//            } else {
//                printf("Response => [ZERO LENGTH MESSAGE]\n");
//            }
            respond();
            return 1;
        } else {
